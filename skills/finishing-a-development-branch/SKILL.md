...
**Normal repo and named-branch worktree — present exactly these 4 options** using the `AskUserQuestion` tool. (`AskUserQuestion` is deferred — load it first via `ToolSearch` with `query: "select:AskUserQuestion"`.)
